<template>
  <div class="page tickets-page px-4 lg:px-8 pt-14 min-h-screen pb-10" :class="loading ? 'flex items-center justify-center' : ''">
    <div class="content" v-if="!loading">
      <div class="lowTickets bg-amber text-center text-black rounded-lg p-2 mb-8" v-show="selectedShow.disponibles <= 10">
        <p>Quedan <b>{{selectedShow.disponibles}}</b> entradas disponibles para esta función</p>
      </div>

      <div class="cartContainer flex gap-8">
...
export default defineComponent({
  ...
  async created() {
    ...
    //get showtimes
    const getShowtimes = utilities.getFromApi("/tickets/" + cinemaId + "/" +fref);
  ...
  methods: {
    addToCart(item: any[any], quantity: number) {
      ...
      //ON EMPTY CART OR ITEM NOT FOUND
      ...
      //reset total tickets count
      this.totalTicketsCount = 0

      //add every item
      this.cart.items.forEach((cartItem: any[any]) => {
        this.cart.total += cartItem.total;
        this.totalTicketsCount += cartItem.cantidad
      });
    },
    async continueToPay() {

      //check if selected 
      if(this.totalTicketsCount > this.selectedShow.disponibles){
        const toast = (this.$refs as any).toast
        toast.show("La cantidad de entradas seleccionadas supera la cantidad disponible")
        throw('Excede entradas disponibles')
      }
